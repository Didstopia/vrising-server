...
## Cap Unity's job worker thread count on high-core-count hosts.
##
## V Rising is a Unity DOTS/ECS game that registers temp/rewindable allocators
## per job-worker thread, per ECS world. Unity defaults the worker count to
## (logical CPUs - 1) and has a hard cap of 2048 registered allocators. On
## high-core machines (e.g. a 28C/56T Xeon, common in TrueNAS/NAS builds) the
## allocator total blows past 2048, Unity hands back invalid allocators, and
## the server dies on startup with:
##   "More than 2048 Allocators are registered. Reduce allocator count"
##   -> windows exception 0xc0000005 -> wine SEH stack overflow
## Capping the worker count keeps the allocator total under the limit. Only the
## job pool is capped; the single-threaded main simulation is unaffected, so
## server performance does not change. Set to empty or 0 to let Unity
## auto-detect (the old, crash-prone behaviour on big CPUs).
## Mechanism (Unity official docs, PlayerCommandLineArguments / JobsUtility):
##   "You can set the job worker count with the command line argument
##    -job-worker-count value for the Editor or standalone Players ... via the
##    boot.config key job-worker-count=value. Unity ignores this setting if the
##    value exceeds the default value that the runtime selects."
## So a LOWER value is always honoured. We apply it two ways for belt-and-braces:
##   1. boot.config key  - parsed at engine init, before any game code, so it
##      cannot be missed or rejected by an argument parser. This is authoritative.
##   2. CLI arg          - also officially supported for standalone players.
## boot.config is regenerated by Steam on update/validate, so we (re)apply it on
## every boot, after the install/update step above has run.
V_RISING_SERVER_JOB_WORKER_COUNT="${V_RISING_SERVER_JOB_WORKER_COUNT-8}"
if [[ "$V_RISING_SERVER_JOB_WORKER_COUNT" =~ ^[1-9][0-9]*$ ]]; then
	echo "Capping Unity job worker threads at ${V_RISING_SERVER_JOB_WORKER_COUNT} (high-core-count >2048-allocator crash workaround).."

	# 1. Authoritative: pin it in Unity's boot.config.
	VR_BOOT_CONFIG="/steamcmd/vrising/VRisingServer_Data/boot.config"
	if [ -f "$VR_BOOT_CONFIG" ]; then
		grep -v '^job-worker-count=' "$VR_BOOT_CONFIG" > "${VR_BOOT_CONFIG}.tmp" 2>/dev/null || true
		echo "job-worker-count=${V_RISING_SERVER_JOB_WORKER_COUNT}" >> "${VR_BOOT_CONFIG}.tmp"
		mv -f "${VR_BOOT_CONFIG}.tmp" "$VR_BOOT_CONFIG"
	else
		echo "WARNING: ${VR_BOOT_CONFIG} not found, relying on the CLI arg only.."
	fi

	# 2. Belt-and-braces: also pass it on the command line.
	V_RISING_SERVER_STARTUP_COMMAND="${V_RISING_SERVER_STARTUP_COMMAND} -job-worker-count ${V_RISING_SERVER_JOB_WORKER_COUNT}"
fi
